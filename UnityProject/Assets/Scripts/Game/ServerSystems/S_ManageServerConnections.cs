...
public class S_ManageServerConnections : JobComponentSystem
{
    ...
    {        
        ...
        JobHandle j_ServerSendVisoreMessages = new J_ServerSendVisorMessages
        {
            RW_Driver = Driver
        }.ScheduleSingle(this, j_ServerSendTextMessages);        

        return j_ServerSendVisoreMessages;
    }
    ...
    struct J_ServerSendVisorMessages : IJobForEachWithEntity_EBC<NMBF_NotifyPlayerVisor, C_NetworkConnection>
    {
        public UdpNetworkDriver RW_Driver;

        public void Execute(Entity entity, int index, DynamicBuffer<NMBF_NotifyPlayerVisor> p_NotifyPlayerText, [ReadOnly] ref C_NetworkConnection p_Connection)
        {
            NMBF_NotifyPlayerVisor t_VisorNotification;

            for (int i = 0; i < p_NotifyPlayerText.Length; i++)
            {
                t_VisorNotification = p_NotifyPlayerText[i];

                using (var writer = new DataStreamWriter(16, Allocator.Temp))
                {
                    writer.Write((int)MessageFromServerTypes.NotifyPlayerVisor);
                    writer.Write(t_VisorNotification.FeedbackHandle);
                    writer.Write(t_VisorNotification.TileHandle);
                    writer.Write(t_VisorNotification.Intensity);
                    p_Connection.Connection.Send(RW_Driver, writer);
                }
            }
